...
const bookAppointment = async (req, res) => {
    ...
    try {
        ...
        // Send appointment confirmation email (do not fail booking if email fails)
        try {
            sendAppointmentConfirmationEmail(userData.email, {
            ...
            console.log("Appointment email error:", emailError?.message || emailError)
        }
...
const sendAppointmentConfirmationEmail = (email, details) => {
    if (!email) return;
    ...
    const { appointmentId, doctorName, doctorSpeciality, slotDate, slotTime, amount } = details || {};
    const readableSlotDate = typeof slotDate === "string" ? slotDate.replaceAll("_", "-") : slotDate;

    const mailOptions = {
        from: "do.not.mail.2fa@gmail.com",
        to: email,
        subject: "Appointment confirmed",
        text: `Your appointment is confirmed.\n\nAppointment ID: ${appointmentId}\nDoctor: ${doctorName}${doctorSpeciality ? ` (${doctorSpeciality})` : ""}\nDate: ${readableSlotDate}\nTime: ${slotTime}\nFee: ${amount}\n\nThank you.`
    };

    auth.sendMail(mailOptions, (error) => {
        if (error) {
            console.log("sendAppointmentConfirmationEmail error:", error.message)
            return
        }
